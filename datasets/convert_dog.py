...
def _get_dataset_filename(dataset_dir, split_name, shard_id):
    output_filename = 'Dog_%s_%05d-of-%05d.tfrecord' % (
...
def generate_datasets(data_root):
    ...
    for index in range(len(train_info)):
        images_file = str(train_info[index][0][0])
        label_name = images_file.split('/')[0]
    ...
    for index in range(len(test_info)):
        images_file = str(test_info[index][0][0])
        label_name = images_file.split('/')[0]
        label = class_names.index(label_name)
...
def run(dataset_dir):
    ...
    _convert_dataset('train', train_dataset,
                     dataset_dir)
    _convert_dataset('test', test_dataset,
                     dataset_dir)

    # _clean_up_temporary_files(dataset_dir)
    print('\nFinished converting the dog dataset!')
